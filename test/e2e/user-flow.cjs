...
async function run() {
  ...
  try {
    ...
    let stored = await page.evaluate(() => Object.fromEntries(Object.entries(window.localStorage)));
    ...
    await dialog.getByLabel("메모").fill("공식 클레버 레시피 E2E 성공");
    ...
    stored = await page.evaluate(() => Object.fromEntries(Object.entries(window.localStorage)));
    ...
    console.log("E2E PASS: mobile registration, persistence, verified Clever, feedback, recipe version, and profile isolation");
  } catch (error) {
