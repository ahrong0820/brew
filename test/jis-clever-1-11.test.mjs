...
test("Jung In-sung Clever recipe keeps only first-party confirmed values", () => {
  assert.ok(recipe);
  assert.equal(recipe.sourceStatus, "reference");
...
test("body and sweet goals prefer 1:11 while balanced official dose prefers official", () => {
  const body = selectBaristaRecipe({
    brewerType: "clever",
    drinkStyle: "hot",
    roastLevel: "medium-light",
    process: "natural",
  ...
  assert.equal(body?.recipe.id, "jis-clever-1-11");

  const sweet = selectBaristaRecipe({
    brewerType: "clever",
    drinkStyle: "hot",
    roastLevel: "medium",
    process: "honey",
  ...
  assert.equal(sweet?.recipe.id, "jis-clever-1-11");

  const balanced = selectBaristaRecipe({
    brewerType: "clever",
    drinkStyle: "hot",
    roastLevel: "medium",
    process: "washed",
  ...
  assert.equal(balanced?.recipe.id, "clever-official-distributor-185");
});

test("personal history can promote 1:11 without forcing it for every balanced user", () => {
  const baseline = rankBaristaRecipes(
    {
      brewerType: "clever",
      drinkStyle: "hot",
      roastLevel: "medium",
  ...
  assert.ok(
    personalized.findIndex((match) => match.recipe.id === "jis-clever-1-11") <
      baseline.findIndex((match) => match.recipe.id === "jis-clever-1-11"),
  );
});

test("profile preserves partial source procedure and labels grinder values as app conversion", () => {
  assert.ok(recipe);
  ...
  assert.equal(result.targetTimeMinSeconds, 200);
  assert.equal(result.targetTimeMaxSeconds, 220);
  ...
  assert.ok(result.reasons.some((reason) => reason.startsWith("[클레버 분쇄 변환]")));
    ...
    result.appliedRules.some((rule) =>
      rule.evidence.some(
        (evidence) =>
          ...
          evidence.applicability === "partial",
      ),
    ),
  );
});
