...
export default function NextBrewAdjustmentDrawer() {
  const [suggestion, setSuggestion] = useState<BrewAdjustmentSuggestion | null>(null);
  ...
  useEffect(() => {
    function onSaved(event: Event) {
      ...
      window.setTimeout(() => {
        const next = createBrewAdjustmentSuggestion(detail.sessionId);
        if (next) {
          setSuggestion(next);
          setMessage(null);
          setApplied(false);
  ...
  function apply() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "보정값을 저장하지 못했습니다.");
    }
  }
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 id="next-brew-title" className="mt-2 text-xl font-bold">다음 추출 조정안</h2>
        ...
        <div className="mt-5 rounded-xl border border-[#dccbb8] bg-white p-4">
          ...
          <h3 className="mt-1 text-lg font-bold text-[#4f3926]">{suggestion.title}</h3>
          ...
          <p className="mt-4 text-sm leading-6 text-[#526055]">{suggestion.reason}</p>
        ...
        {message && (
          <p role="status" className={`mt-4 rounded-lg px-3 py-2 text-sm ${applied ? "bg-[#eaf3ee] text-[#245647]" : "bg-[#fff0e8] text-[#8a4d24]"}`}>
            {message}
          </p>
        )}
        ...
        <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
          {suggestion.canApply && !applied ? (
            <>
              <button type="button" onClick={close} className="h-11 rounded-lg border border-[#c8d0c5] bg-white px-4 text-sm font-semibold text-[#526055] hover:bg-[#f8faf7]">
                이번만 참고
              </button>
              <button type="button" onClick={apply} className="flex h-11 items-center justify-center gap-2 rounded-lg bg-[#8a623d] px-5 text-sm font-bold text-white hover:bg-[#735033] focus:outline-none focus:ring-2 focus:ring-[#8a623d] focus:ring-offset-2">
                <Check aria-hidden="true" size={17} /> 다음 추천에 반영
              </button>
            </>
          ) : (
            <button type="button" onClick={close} className="flex h-11 items-center justify-center gap-2 rounded-lg bg-[#2f6f5f] px-5 text-sm font-bold text-white hover:bg-[#25594c]">
